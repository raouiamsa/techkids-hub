...
const LevelBadge = ({ level }: { level: string }) => {
  const map: Record<string, { label: string; cls: string }> = {
    BEGINNER: { label: 'Débutant', cls: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' },
    INTERMEDIATE: { label: 'Intermédiaire', cls: 'bg-amber-500/10 text-amber-500 border-amber-500/20' },
    ADVANCED: { label: 'Avancé', cls: 'bg-rose-500/10 text-rose-500 border-rose-500/20' },
  };
  const config = map[level] ?? map.BEGINNER;
  return (
    <span className={`px-4 py-1.5 rounded-full text-sm font-medium border ${config.cls}`}>
      Niveau {config.label}
    </span>
  );
};
