...
const CourseLevelBadge = ({ level }: { level: string }) => {
  ...
  return (
    <Badge variant={config.variant} className="px-3 py-1 rounded-full text-xs font-medium">
      {config.label}
    </Badge>
  );
};
...
import { Card, CardContent } from '@org/ui-components';
        ...
        {coursesList.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {coursesList.map((course) => (
              ...
              >
                <Card className="flex flex-col h-full rounded-3xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  {/* Decorative header */}
