...
def process_pdf(file_path: str, source_id: str):
    """
    """Analyse complète d'un PDF : Texte + Images + OCR.
    Optimisé pour un Recall élevé (Chunks 1500/300 + Visual Context).
    """
    """if not os.path.exists(file_path):
        print(f"Erreur : Le fichier {file_path} n'existe pas.")
        return
    ...
    # [OPTIMISATION RECALL]: Augmentation de la taille des segments pour plus de contexte
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=1500, # Augmenté de 1200 à 1500
        chunk_overlap=300, # Augmenté de 200 à 300 pour éviter les coupures d'information
        separators=["\n\n", "\n", ".", "!", "?", " "]
    )
    texts = text_splitter.split_documents(documents)
    ...
    for page_idx in tqdm(range(pages_count), desc="Analyse des pages"):
        ...
        try:
            ...
            if any(kw in page.get_text().lower() for kw in FIGURE_KEYWORDS):
                description = _florence_task(image_pil, "<DETAILED_CAPTION>")
                if description:
                    # [OPTIMISATION RECALL]: Ajout d'un extrait du texte de la page pour lier l'image à son contexte[cite: 3]
                    page_text_snippet = page.get_text()[:300].replace('\n', ' ')
                    visual_contexts.append(Document(
                        page_content=f"[CONTEXTE PAGE {page_idx+1}]: {page_text_snippet}... [DESCRIPTION VISUELLE]: {description}", 
                        metadata={
                            "course_id": source_id, 
                            "source_name": file_name, 
                            "page": page_idx+1, 
    ...
    for t in texts:
        ...
        if page_num - 1 in ocr_enrichments: 
            t.page_content += f"\n\n[TEXTE EXTRAIT DE L'IMAGE PAGE {page_num}] : {ocr_enrichments[page_num - 1]}"
        final_docs.append(t)
        
    final_docs.extend(visual_contexts)

    print(f" Sauvegarde de {len(final_docs)} segments dans ChromaDB...")
    Chroma.from_documents(documents=final_docs, embedding=EMBEDDINGS, persist_directory=CHROMA_PERSIST_DIR)
    
    report_progress(source_id, 100)
    print(f" Ingestion terminée. Recall optimisé via chunks de 1500 chars[cite: 3].")"""